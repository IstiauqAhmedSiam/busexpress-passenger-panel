...
const DroppingSelectionPage = () => {
    const router = useRouter();
    const { numberOfTickets, selectedDroppingPoint, setSelectedDroppingPoint, totalPrice, selectedSeats, timerRemaining } = useTicketProvider(); 
    ...
    const handleDroppingSelect = (id) => {
        setSelectedDroppingPoint(droppingPoints.find(p => p.id === id));
    };

    const handleConfirmBooking = () => {
        if (selectedDroppingPoint.name) {
            router.push("/ticket/book/passenger-info");
        } else {
        ...
        <>
            <div className="flex justify-between items-center bg-white p-4 rounded-md shadow-[0_-5px_13px_0_rgba(0,0,0,.25)] fixed left-0 bottom-0 w-full">
                ...
                <button
                    ...
                    disabled={!selectedDroppingPoint.name}
                >
                    Confirm
                </button>
            </div>

            <SelectTripPoint remainingTime={timerRemaining} backLink="/ticket/book/boarding" name="Dropping" points={droppingPoints} selectedPoint={selectedDroppingPoint.id} onClick={handleDroppingSelect}/>
        </>
    );
};

export default DroppingSelectionPage;